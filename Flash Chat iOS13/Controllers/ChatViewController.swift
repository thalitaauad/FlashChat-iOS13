...
class ChatViewController: UIViewController {
    ...
    let db = Firestore.firestore() //referencia ao db
    ...
    override func viewDidLoad() {
        ...
        title = K.appName
        navigationItem.hidesBackButton = true //oculta o botao BACK nessa tela
        
        tableView.register(UINib(nibName: K.cellNibName, bundle: nil), forCellReuseIdentifier: K.cellIdentifier)
        
        loadMessages()
    }
    //obter docs chamando .addSnapshotListener
    func loadMessages() {
        ...
        db.collection(K.FStore.collectionName).order(by: K.FStore.dateField).addSnapshotListener { querySnapshot , error in
            ...
            if let e = error {
                print("Error to retrieving data from firestore, \(e)")
            } else {
                //quey -> consulta optional
                if let snapshotDocuments = querySnapshot?.documents {
                    for doc in snapshotDocuments {
                        ...
                        if let messageSender = data[K.FStore.senderField] as? String, let messageBody = data[K.FStore.bodyField] as? String {
                            ...
                            //metodo para acionar atualização para tableView
